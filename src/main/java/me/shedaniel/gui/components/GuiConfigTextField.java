...
public class GuiConfigTextField extends Gui implements IGuiEventListener {
	...
	private TextFieldInputType inputType;
	
	public GuiConfigTextField(TextFieldInputType inputType, int componentId, FontRenderer fontrendererObj, int x, int y, int par5Width, int par6Height) {
		this(inputType, componentId, fontrendererObj, x, y, par5Width, par6Height, (GuiConfigTextField) null);
	}
	
	public GuiConfigTextField(TextFieldInputType inputType, int p_i49853_1_, FontRenderer p_i49853_2_, int p_i49853_3_, int p_i49853_4_, int p_i49853_5_, int p_i49853_6_, @Nullable GuiConfigTextField p_i49853_7_) {
	...
	public boolean isAllowedCharacter(char character) {
		switch (inputType) {
			...
			case NUMBER:
				return Character.isDigit((int) character) || character == 45;
			case NUMBER_WITH_DECIMALS:
				return Character.isDigit((int) character) || character == 45 || character == 46;
		}
		return true;
	}
	...
	public String filterAllowedCharacters(String input) {
		...
		for (int i = 0; i < input.toCharArray().length; i++) {
			char c0 = input.toCharArray()[i];
			if (isAllowedCharacter(c0)) {
				if (c0 == 46 && inputType == TextFieldInputType.NUMBER_WITH_DECIMALS) {
					if (!usedDecimal) {
						usedDecimal = true;
						stringbuilder.append(c0);
					}
				} else if (c0 == 45 && (inputType == TextFieldInputType.NUMBER_WITH_DECIMALS || inputType == TextFieldInputType.NUMBER)) {
					if (this.cursorPosition == 0 && getCharAt(0, (char) 0) != 45)
						stringbuilder.append(c0);
	...
	private char getCharAt(int i, char defaultAnswer) {
		try {
	...
	public enum TextFieldInputType {
		TEXT, NUMBER, NUMBER_WITH_DECIMALS;
	}
